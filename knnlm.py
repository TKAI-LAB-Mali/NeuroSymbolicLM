...
class KNNWrapper(object):
    def __init__(self, dstore_size, dstore_damb, dstore_dir, dimension, 
            ...
            k=512, t=10, lmbda1=0.25, knn_temp=1.0, probe=32):
    ...
    def break_into(self, model):
        ...
        last_layer_device = model.hf_device_map['lm_head']
    ...
    def post_forward_hook(self, module, input, output):
        ...
        if self.unique:
            ...
            interpolated_scores = KNNWrapper.interpolate(knn_log_probs, lm_logits, self.lmbda1) # (nonpad, vocab)
        ...
        for que, fais_obj, dsa in zip(unbatch_queries, self.faiss_objects, self.dstore_size_disamb):
            ...
            dists, knns = self.get_knns(que) # (nonpad batch * time, k)
        ...
        interpolated_scores = KNNWrapper.interpolate(torch.cat(knn_log_probs_list), lm_logits, self.lmbda1) # (nonpad, vocab)
        output[nonpad_mask] = interpolated_scores
        return output 
    ...
    @staticmethod
    def interpolate(knn_log_probs, lm_log_probs, lmbda1):
